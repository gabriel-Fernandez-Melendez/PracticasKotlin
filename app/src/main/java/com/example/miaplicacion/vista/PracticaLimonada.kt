...
fun PantallaDeLimonada(modifier: Modifier = Modifier) { // el centrado del texto se hace dento de la propia collum
    ...
    Column(
        ...
        Text(modifier = Modifier.background(Color.Yellow).fillMaxWidth(),
        ...
        when(veces.value){
            ...
            3->Column(modifier.fillMaxWidth().fillMaxHeight(),
                horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center){
                Button(onClick = {veces.value++}) {
                    Image(painter = painterResource(id = R.drawable.lemon_drink), contentDescription = "rt")

                }
                Text(text = "click en la limonada para beberla")
            }
            4->Column(modifier.fillMaxWidth().fillMaxHeight(),
                horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center){
                Button(onClick = {veces.value=1}) {
                    Image(painter = painterResource(id = R.drawable.lemon_restart), contentDescription = "rt")
